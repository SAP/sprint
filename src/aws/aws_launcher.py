...
$ python3 crypten/scripts/aws_launcher.py \
    ...
    --instances=i-038dd14b9383b9d79,i-08f057b9c03d4a916 \
    --aux_files=crypten/examples/mpc_lineaPypipr_svm/mpc_linear_svm.py \
    crypten/examples/mpc_linear_svm/launcher.py \
        --features 50 \
        --examples 100 \
        --epochs 50 \
...
def upload_data_folder_recursively(instance_id, client, localpath, remotepath):
    dataset_name = localpath.split("/")[-2]
    model_type = localpath.split("/")[-3]
    run_command(instance_id, client, f"mkdir -p {remotepath}/{dataset_name}")

    dataset_types = ["train", "val", "test"]

    for dataset_type in dataset_types:
        run_command(instance_id, client, f"mkdir -p {remotepath}/{model_type}/{dataset_name}/{dataset_name}_{dataset_type}_dataset")
        new_path = os.path.join(localpath, f"{dataset_name}_{dataset_type}_dataset")
        for files in os.listdir(new_path):
                upload_file(instance_id, client, os.path.join(new_path, files), os.path.join(remotepath, f"{model_type}/{dataset_name}/{dataset_name}_{dataset_type}_dataset", files))




    ftp_client = client.open_sftp()
    print(f"Uploading folder `{localpath}` to {instance_id}...")
    ftp_client.put(localpath, remotepath)

def upload_folder_recursively(instance_id, client, localpath, remotepath):
    folder_name = localpath.split("/")[-2]

    run_command(instance_id, client, f"mkdir -p {remotepath}/{folder_name}")
    print(f"Execute command: mkdir -p {remotepath}/{folder_name}")

    for root, dirs, files in os.walk(localpath):
        # Create corresponding directories on the remote path
        for dir_name in dirs:
            local_dir = os.path.join(root, dir_name)
            relative_dir = os.path.relpath(local_dir, localpath)
            remote_dir = os.path.join(remotepath, folder_name, relative_dir)
            run_command(instance_id, client, f"mkdir -p {remote_dir}")
            print(f"Execute command: mkdir -p {remote_dir}")

        # Upload files
        for file_name in files:
            local_file = os.path.join(root, file_name)
            relative_file = os.path.relpath(local_file, localpath)
            remote_file = os.path.join(remotepath, folder_name, relative_file)
            print(f"Uploading `{local_file}` to remote path `{remote_file}`")
            upload_file(instance_id, client, local_file, remote_file)


def upload_folder_content_recursively(instance_id, client, localpath, remotepath):
    # Contrarily to upload_folder_recursively, this function uploads the CONTENT of the folder, and not the folder and subfolder
    folder_name = localpath.split("/")[-2]
    ...
    for root, dirs, files in os.walk(localpath):
        ...
        for dir_name in dirs:
            local_dir = os.path.join(root, dir_name)
            relative_dir = os.path.relpath(local_dir, localpath)
            remote_dir = os.path.join(remotepath, relative_dir)
            run_command(instance_id, client, f"mkdir -p {remote_dir}")
            print(f"Execute command: mkdir -p {remote_dir}")
        ...
        for file_name in files:
            local_file = os.path.join(root, file_name)
            relative_file = os.path.relpath(local_file, localpath)
            remote_file = os.path.join(remotepath, relative_file)
            print(f"Uploading `{local_file}` to remote path `{remote_file}`")
            upload_file(instance_id, client, local_file, remote_file)
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as uploaders:
        for instance_id, client in client_dict.items():
            ...
            if args.src_folder is not None:
                print("Uploading source folder")
                uploaders.submit(
                    upload_folder_content_recursively,
                    instance_id,
                    client,
                    args.src_folder,
                    os.path.join(remote_dir),
                )
            
            #if args.utils_folder is not None:
            #    print("Uploading utils folder")
            #        # Upload utils folder
            #    uploaders.submit(
            ...
            if args.data_folder is not None:
                # Upload data folder
                uploaders.submit(
                    upload_data_folder_recursively,
                    instance_id,
                    client,
                    args.data_folder,
                    os.path.join(remote_dir),
    ...
    #if len(regions) > 1:
    #    # multiple regions, require public ip address
    #    environment = {
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=world_size) as executor:
        ...
        for instance_id, client in client_dict.items():
            ...
            if len(regions) > 1:
                environment["RENDEZVOUS"] = f"env://{master_instance.public_ip_address}:{args.master_port}"
                environment["MASTER_ADDR"] = master_instance.public_ip_address
            environment["RANK"] = str(rank)
